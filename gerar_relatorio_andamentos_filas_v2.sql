...
DROP PROCEDURE IF EXISTS `gerar_relatorio_andamentos_filas_v2`$$

CREATE PROCEDURE `gerar_relatorio_andamentos_filas_v2`(
	IN tgid INT(11),
	...
	IN rp_type VARCHAR(255),
	IN rp_filter_step VARCHAR(255)
...
SET @report_filter_step = rp_filter_step;
SET @report_type = rp_type;
...
WHERE ts.int_placecleaninglist_group_key = @terminal_group_id AND ts.vch_type IN ('active','passive')
GROUP BY ts.int_placecleaninglist_group_key;
SET @tmp_query_2 = CONCAT('SELECT Stts.*, ', @sql_columns2 , ' FROM (',@tmp_query_1_2,') AS Stts ',
CASE 
WHEN @report_type = 'Normal' THEN CONCAT('WHERE (', @sql_columns3, ')')
WHEN @report_type = 'Step' THEN CONCAT('WHERE Stts.`Etapa ',@report_filter_step,'` IS NOT NULL')
ELSE CONCAT('WHERE (', @sql_columns3, ')')
END,IF(LENGTH(@report_filter_person) = 0, '', CONCAT(' AND Stts.Participantes like "%',@report_filter_person,'%" ')), IF(LENGTH(@report_filter_person_responsible) = 0, '', CONCAT(' AND `Stts`.`Lancado Por` like "%',@report_filter_person_responsible,'%" ')));
