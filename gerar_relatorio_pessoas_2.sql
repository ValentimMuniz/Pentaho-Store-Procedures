...
DROP PROCEDURE IF EXISTS `gerar_relatorio_pessoas_2`$$

CREATE PROCEDURE gerar_relatorio_pessoas_2(IN tgid INT(11),IN start_time VARCHAR(255),IN end_time VARCHAR(255), IN order_field VARCHAR(255))
BEGIN
...
SET @tmp_query_1 = CONCAT(
	'SELECT t1.int_placecleaninglist_key AS CleaningKey, place.vch_name as PlaceName,
    GROUP_CONCAT(DISTINCT ct.vch_name SEPARATOR ",") as Tipos_Limpeza,
    ', @sql_columns1, '
...
CASE 	
	WHEN (tsfs.int_start_from_terminal_step_key > 0 AND tsfs.int_end_on_terminal_step_key > 0)
	THEN CONCAT('SUM(TIMESTAMPDIFF(SECOND, `',tss2.vch_name,'`, `',tss1.vch_name,'`)) AS "Total_Tempo_Terminal_',tss2.vch_name,'_',tss1.vch_name,'", COUNT("Tempo_Terminal_',tss2.vch_name,'_',tss1.vch_name,'") AS "Quantidade_Terminal_',tss2.vch_name,'_',tss1.vch_name,'"')
END ORDER BY tss.int_order
)) AS diff_field
INTO @sql_columns4_1
FROM tbl_hlt_placecleaninglist_step_flows tsfs
...
CASE 	
	WHEN (tsfs.int_start_from_terminal_step_key > 0 AND tsfs.int_end_on_terminal_step_key > 0)
	THEN CONCAT('`Tempo_Terminal_',tss2.vch_name,'_',tss1.vch_name,'`')
...
SET @tmp_query_3 = CONCAT('
        ...
        GROUP_CONCAT(DISTINCT Tipos_Limpeza SEPARATOR ",") as TiposLimpeza2,
		(SELECT COALESCE(NULLIF(pr.vch_name, ""),pr.vch_name) FROM tbl_hlt_person pr WHERE pr.int_key = Times.ColaboradorKey) as Colaborador, 
...
CASE 	
	WHEN (tsfs.int_start_from_terminal_step_key > 0 AND tsfs.int_end_on_terminal_step_key > 0)
	THEN CONCAT('AVG(`Total_Tempo_Terminal_',tss2.vch_name,'_',tss1.vch_name,'`) as "Tempo Medio Trabalhado Terminal(',tss2.vch_name,'-',tss1.vch_name,') por dia"',
	',AVG(`Quantidade_Terminal_',tss2.vch_name,'_',tss1.vch_name,'`) as "Quantidade Media Terminal(',tss2.vch_name,'-',tss1.vch_name,')"',
	',AVG(`Total_Tempo_Terminal_',tss2.vch_name,'_',tss1.vch_name,'`/`Quantidade_Terminal_',tss2.vch_name,'_',tss1.vch_name,'`) as "Tempo Medio Cada Terminal(',tss2.vch_name,'-',tss1.vch_name,')"')
END ORDER BY tss.int_order
)) AS diff_field
INTO @sql_columns5
FROM tbl_hlt_placecleaninglist_step_flows tsfs
...
SET @tmp_query_4 = CONCAT('
	SELECT 
	Totais.Colaborador, 
    GROUP_CONCAT(Totais.TiposLimpeza2 SEPARATOR ",") as "Tipos de Limpeza",
	SUM(Totais.QuantidadeTerminal) as "Quantidade de Terminais",
	COUNT(Totais.TempoTotalTrabalhadoTerminal) as "Dias Trabalhados",  
	AVG(Totais.QuantidadeConcorrente*600) as "Tempo Médio Trabalhado Concorrente por Dia", 
	AVG(Totais.QuantidadeConcorrente) as "Quantidade Média Concorrentes", ',@sql_columns5,',
	AVG(IFNULL(Totais.TempoTotalTrabalhadoTerminal, 0)) as "Tempo Médio Trabalhado Terminal por Dia",
